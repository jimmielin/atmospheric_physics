...
subroutine modal_aero_calcsize_run( &
   ...
   real(kind_phys), intent(in) :: deltat            ! model time-step size (s)
   ...
   ! Mode geometry
   real(kind_phys), intent(in) :: dgnum_amode(:)    ! default geometric mean diameter (ntot_amode)
   real(kind_phys), intent(in) :: dgnumlo_amode(:)  ! lower bound dgnum (ntot_amode)
   real(kind_phys), intent(in) :: dgnumhi_amode(:)  ! upper bound dgnum (ntot_amode)

   ! Derived mode quantities
   real(kind_phys), intent(in) :: alnsg_amode(:)    ! ln(sigmag) for each mode (ntot_amode)
   real(kind_phys), intent(in) :: voltonumb_amode(:)    ! volume-to-number (ntot_amode)
   real(kind_phys), intent(in) :: voltonumblo_amode(:)  ! vol-to-num at dgnumlo (ntot_amode)
   real(kind_phys), intent(in) :: voltonumbhi_amode(:)  ! vol-to-num at dgnumhi (ntot_amode)

   ! Species densities
   real(kind_phys), intent(in) :: specdens_amode(:,:)   ! species densities (nspec_max,ntot_amode)
   ...
   ! Atmospheric state
   real(kind_phys), intent(in) :: pdel(:,:)         ! pressure thickness (ncol,pver)
   ...
   real(kind_phys), parameter :: third = 1.0_kind_phys/3.0_kind_phys
   real(kind_phys) :: delnum_a2, delnum_c2            !  work variables
   real(kind_phys) :: delnum_a3, delnum_c3, delnum_t3 !  work variables
   real(kind_phys) :: deltatinv                     ! 1/deltat
   real(kind_phys) :: dgncur_c(ncol,pver,ntot_amode)
   real(kind_phys) :: dgnyy, dgnxx                  ! dgnumlo/hi of current mode
   real(kind_phys) :: drv_a, drv_c, drv_t           ! dry volume (cm3/mol_air)
   real(kind_phys) :: drv_t0
   real(kind_phys) :: drv_a_noxf, drv_c_noxf, drv_t_noxf
   real(kind_phys) :: drv_a_acc, drv_c_acc
   real(kind_phys) :: drv_a_accsv(ncol,pver), drv_c_accsv(ncol,pver)
   real(kind_phys) :: drv_a_aitsv(ncol,pver), drv_c_aitsv(ncol,pver)
   real(kind_phys) :: drv_a_sv(ncol,pver,ntot_amode), drv_c_sv(ncol,pver,ntot_amode)
   real(kind_phys) :: dryvol_a(ncol,pver)          ! interstital aerosol dry
   ! volume (cm^3/mol_air)
   real(kind_phys) :: dryvol_c(ncol,pver)          ! activated aerosol dry volume
   real(kind_phys) :: duma, dumb, dumc, dumd        ! work variables
   real(kind_phys) :: dumfac, dummwdens             ! work variables
   real(kind_phys) :: frelaxadj                     ! relaxation factor applied
   ! to size bounds
   real(kind_phys) :: fracadj                       ! deltat/tadj
   real(kind_phys) :: num_a0, num_c0, num_t0        ! initial number (#/mol_air)
   real(kind_phys) :: num_a1, num_c1                ! working number (#/mol_air)
   real(kind_phys) :: num_a2, num_c2, num_t2        ! working number (#/mol_air)
   real(kind_phys) :: num_a, num_c, num_t           ! final number (#/mol_air)
   real(kind_phys) :: num_t_noxf
   real(kind_phys) :: numbnd                        ! bounded number
   real(kind_phys) :: num_a_acc, num_c_acc
   real(kind_phys) :: num_a_accsv(ncol,pver), num_c_accsv(ncol,pver)
   real(kind_phys) :: num_a_aitsv(ncol,pver), num_c_aitsv(ncol,pver)
   real(kind_phys) :: num_a_sv(ncol,pver,ntot_amode), num_c_sv(ncol,pver,ntot_amode)
   real(kind_phys) :: pdel_fac                      !
   real(kind_phys) :: tadj                          ! adjustment time scale
   real(kind_phys) :: tadjinv                       ! 1/tadj
   real(kind_phys) :: v2ncur_a(ncol,pver,ntot_amode)
   real(kind_phys) :: v2ncur_c(ncol,pver,ntot_amode)
   real(kind_phys) :: v2nyy, v2nxx, v2nzz           ! voltonumblo/hi of current mode
   real(kind_phys) :: v2nyyrl, v2nxxrl              ! relaxed voltonumblo/hi
   real(kind_phys) :: xfercoef
   real(kind_phys) :: xfercoef_num_acc2ait, xfercoef_vol_acc2ait
   real(kind_phys) :: xfercoef_num_ait2acc, xfercoef_vol_ait2acc
   real(kind_phys) :: xferfrac_num_acc2ait, xferfrac_vol_acc2ait
   real(kind_phys) :: xferfrac_num_ait2acc, xferfrac_vol_ait2acc
   real(kind_phys) :: xfertend, xfertend_num(2,2)
   ...
   do n = 1, ntot_amode
      ...
      do k=top_lev,pver
         do i=1,ncol
            ...
            dryvol_a(i,k) = 0.0_kind_phys
            dryvol_c(i,k) = 0.0_kind_phys
         end do
      end do
      ...
      do l1 = 1, nspec_amode(n)
         ...
         dummwdens = 1.0_kind_phys / specdens_amode(l1,n)
         la = lmassptr_amode(l1,n)
         do k=top_lev,pver
            do i=1,ncol
               dryvol_a(i,k) = dryvol_a(i,k)    &
                  + max(0.0_kind_phys,q(i,k,la))*dummwdens
            end do
         end do
         ...
         do k=top_lev,pver
            do i=1,ncol
               dryvol_c(i,k) = dryvol_c(i,k)    &
                  + max(0.0_kind_phys,q_cw(i,k,lc))*dummwdens
            end do
         end do
      end do
      ...
      frelaxadj = 27.0_kind_phys
      dumfac = exp(4.5_kind_phys*alnsg_amode(n)**2)*pi/6.0_kind_phys
      ...
      do  k = top_lev, pver
         do  i = 1, ncol
            ...
            if (do_adjust) then
               ...
               !
               if ((drv_a <= 0.0_kind_phys) .and. (drv_c <= 0.0_kind_phys)) then
               ...
               else if (drv_c <= 0.0_kind_phys) then
               ...
               else if (drv_a <= 0.0_kind_phys) then
                  ...
                  delnum_a3 = 0.0_kind_phys
                  delnum_c3 = 0.0_kind_phys
            ...
            jac = 1
            qsrflx(i,lna,1,jac) = qsrflx(i,lna,1,jac) + max(0.0_kind_phys,dqdt(i,k,lna))*pdel_fac
            qsrflx(i,lna,2,jac) = qsrflx(i,lna,2,jac) + min(0.0_kind_phys,dqdt(i,k,lna))*pdel_fac

            if (drv_c > 0.0_kind_phys) then
            ...
            jac = 2
            qsrflx(i,lnc,1,jac) = qsrflx(i,lnc,1,jac) + max(0.0_kind_phys,dqdt_cw(i,k,lnc))*pdel_fac
            qsrflx(i,lnc,2,jac) = qsrflx(i,lnc,2,jac) + min(0.0_kind_phys,dqdt_cw(i,k,lnc))*pdel_fac
   ...
   if ( do_aitacc_transfer ) then
      ...
      do  k = top_lev, pver
         do  i = 1, ncol
            ...
            pdel_fac = pdel(i,k)/gravit   ! = rho*dz
            xfertend_num(:,:) = 0.0_kind_phys

            ! compute aitken --> accum transfer rates
            ixfer_ait2acc = 0
            xfercoef_num_ait2acc = 0.0_kind_phys
            xfercoef_vol_ait2acc = 0.0_kind_phys

            drv_t = drv_a_aitsv(i,k) + drv_c_aitsv(i,k)
            num_t = num_a_aitsv(i,k) + num_c_aitsv(i,k)
            if (drv_t > 0.0_kind_phys) then
               if (num_t < drv_t*v2nzz) then
                  ixfer_ait2acc = 1
                  if (num_t < drv_t*voltonumb_amode(nacc)) then
                     xferfrac_num_ait2acc = 1.0_kind_phys
                     xferfrac_vol_ait2acc = 1.0_kind_phys
                  else
                     ...
                     if ((xferfrac_num_ait2acc <= 0.0_kind_phys) .or.   &
                        (xferfrac_vol_ait2acc <= 0.0_kind_phys)) then
                        xferfrac_num_ait2acc = 0.0_kind_phys
                        xferfrac_vol_ait2acc = 0.0_kind_phys
                     else if ((xferfrac_num_ait2acc >= 1.0_kind_phys) .or.   &
                        (xferfrac_vol_ait2acc >= 1.0_kind_phys)) then
                        xferfrac_num_ait2acc = 1.0_kind_phys
                        xferfrac_vol_ait2acc = 1.0_kind_phys
                     end if
                  end if
            ...
            ixfer_acc2ait = 0
            xfercoef_num_acc2ait = 0.0_kind_phys
            xfercoef_vol_acc2ait = 0.0_kind_phys

            drv_t = drv_a_accsv(i,k) + drv_c_accsv(i,k)
            num_t = num_a_accsv(i,k) + num_c_accsv(i,k)
            drv_a_noxf = 0.0_kind_phys
            drv_c_noxf = 0.0_kind_phys
            if (drv_t > 0.0_kind_phys) then
               if (num_t > drv_t*v2nzz) then
                  do l1 = 1, nspec_amode(nacc)
            ...
            if (drv_t > 0.0_kind_phys) then
               if (num_t > drv_t*v2nzz) then
                  ...
                  if (num_t > drv_t*voltonumb_amode(nait)) then
                     xferfrac_num_acc2ait = 1.0_kind_phys
                     xferfrac_vol_acc2ait = 1.0_kind_phys
                  else
                     ...
                     if ((xferfrac_num_acc2ait <= 0.0_kind_phys) .or.   &
                        (xferfrac_vol_acc2ait <= 0.0_kind_phys)) then
                        xferfrac_num_acc2ait = 0.0_kind_phys
                        xferfrac_vol_acc2ait = 0.0_kind_phys
                     else if ((xferfrac_num_acc2ait >= 1.0_kind_phys) .or.   &
                        (xferfrac_vol_acc2ait >= 1.0_kind_phys)) then
                        xferfrac_num_acc2ait = 1.0_kind_phys
                        xferfrac_vol_acc2ait = 1.0_kind_phys
                     end if
                  end if
...
!-----------------------------------------------------------------------
   ...
   real(kind_phys),         intent(in)  :: dgncur_a(:,:,:)         ! dry number mode diameter (m)

   real(kind_phys),         intent(out) :: hygro(:,:,:)            ! volume-weighted mean hygroscopicity (--)
   real(kind_phys),         intent(out) :: dryvol(:,:,:)           ! single-particle-mean dry volume (m3)
   real(kind_phys),         intent(out) :: dryrad(:,:,:)           ! dry volume mean radius of aerosol (m)
   real(kind_phys),         intent(out) :: drymass(:,:,:)          ! single-particle-mean dry mass (kg)
   real(kind_phys),         intent(out) :: so4dryvol(:,:,:)        ! single-particle-mean so4 dry volume (m3)
   real(kind_phys),         intent(out) :: naer(:,:,:)             ! aerosol number MR (#/kg-air)
   character(len=*), intent(out) :: errmsg
   integer,          intent(out) :: errflg

   ! local variables
   real(kind_phys), parameter :: third = 1._kind_phys/3._kind_phys
   real(kind_phys), parameter :: pi43 = pi*4._kind_phys/3._kind_phys

   integer  :: i, k, l, m
   integer  :: nmodes, nspec

   real(kind_phys) :: specdens
   real(kind_phys) :: spechygro, spechygro_1
   real(kind_phys) :: sigmag
   real(kind_phys) :: duma, dumb
   real(kind_phys) :: alnsg

   real(kind_phys) :: v2ncur_a
   real(kind_phys) :: drydens               ! dry particle density  (kg/m^3)

   real(kind_phys) :: maer(ncol, pver)
   real(kind_phys) :: dryvolmr(ncol, pver)
   real(kind_phys) :: so4dryvolmr(ncol, pver)
   ...
   do m = 1, nmodes
      ...
      maer(:,:)        = 0._kind_phys
      dryvolmr(:,:)    = 0._kind_phys
      so4dryvolmr(:,:) = 0._kind_phys
      ...
      do k = top_lev, pver
         do i = 1, ncol
            ...
            if (dryvolmr(i,k) > 1.0e-30_kind_phys) then
            ...
            dryvol(i,k,m)   = 1.0_kind_phys/v2ncur_a             ! m3/particle
