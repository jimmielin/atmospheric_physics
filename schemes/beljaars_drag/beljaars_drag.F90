...
!! \htmlinclude beljaars_drag_run.html
  ...
  pure subroutine beljaars_drag_run( &
    ...
    real(kind_phys),    intent(out) :: taux_beljaars(:)      ! surface zonal wind stress [N m-2]
    real(kind_phys),    intent(out) :: tauy_beljaars(:)      ! surface meridional wind stress [N m-2]
    ...
    if(.not. do_beljaars) then
      ...
      drag(:,:) = 0._kind_phys
      taux_beljaars(:)   = 0._kind_phys
      tauy_beljaars(:)   = 0._kind_phys
      return
    end if
    ...
    !
    ! taux_beljaars, tauy_beljaars stresses generated by Beljaars drag here
    ...
    taux_beljaars(:) = 0.0_kind_phys
    tauy_beljaars(:) = 0.0_kind_phys
    do k = 1, pver
      do i = 1, ncol
        taux_beljaars(i) = taux_beljaars(i) + drag(i, k) * u(i, k) * delp(i, k) / gravit
        tauy_beljaars(i) = tauy_beljaars(i) + drag(i, k) * v(i, k) * delp(i, k) / gravit
      end do
    end do
