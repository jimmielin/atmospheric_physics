...
! - Part A: in-cloud liquid/ice water paths (iclwp/iciwp), optionally
!   accounting for convective in-cloud water via convective_cloud_water.
!   Ported verbatim from CAM src/physics/cam/cloud_diagnostics.F90
!   (cloud_diagnostics_calc, two-moment branch), CAM hash 79f07a068.
!   Despite the CAM module name these are load-bearing radiation inputs,
!   not history diagnostics.
! - Part B: snow/graupel-adjusted cloud fractions and in-cloud snow/graupel
!   water paths (cldfsnow/icswp, cldfgrau/icgrauwp). Ported verbatim from
!   CAM src/physics/cam/micro_pumas_cam.F90 (in-cloud diagnostics block),
!   same hash. The graupel block is computed unconditionally: CAM gates it
!   on micro_mg_version > 2, and the SIMA PUMAS port is v3-only.
!
! History diagnostics for SIMA are not yet implemented but they are intent(out)
! arguments here for future implementation.
module cloud_water_paths
  ...
  ! Minimum cloud fraction floor for in-cloud conversions; equals PUMAS
  ! micro_pumas_utils::mincld and the literal in CAM cloud_diagnostics.
...
!! \htmlinclude cloud_water_paths_run.html
  subroutine cloud_water_paths_run( &
    ...
    integer,          intent(in)  :: conv_water_in_rad  ! 0==> No; else use convective water in radiation
    ...
    if( conv_water_in_rad /= 0 ) then
       ...
       allcld_liq(:ncol,top_lev:pver) = ls_liq(:ncol,top_lev:pver)  ! Grid-ave all cloud liquid
       allcld_ice(:ncol,top_lev:pver) = ls_ice(:ncol,top_lev:pver)  !           "        ice
    end if
