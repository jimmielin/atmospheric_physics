...
! stratiform condensate, controlled by the conv_water_nl namelist.
!
! This is shared portable code from CAM extracted into src/physics/cam/convective_cloud_water.F90
! and swept r8->kind_phys;
! it was originally conv_water.F90::conv_water_4rad
!
! Notes:
! - one_mom_clouds selects the one-moment (RK) ice absorptivity (rei used
!   unclamped); CAM passes microp_scheme=='RK', CAM-SIMA hardcodes .false.
!   in the registry until we finalize and validate the RK+RRTMGP configuration.
!   (TODO CAM4)
! - History diagnostics are to be done but they are intent(out) from here.
...
!! \htmlinclude convective_cloud_water_run.html
  subroutine convective_cloud_water_run( &
    ...
    parameter( kabsl = 0.090361_kind_phys, ic_limit = 1.e-12_kind_phys )
