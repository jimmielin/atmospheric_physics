! Adapted from CAM/src/chemistry/pp_trop_mozart/mo_imp_sol.F90
! MOD for CAM-SIMA: use ccpp_kinds instead of shr_kind_mod
! MOD for CAM-SIMA: removed cam_logfile(iulog) - use unit 0 (stderr) for error messages
! MOD for CAM-SIMA: removed time_manager(get_nstep), perf_mod(t_startf/t_stopf)
module mo_imp_sol
...
contains
  subroutine imp_slv_inti
    ...
    if( o3a_ndx > 0 ) then
       eps(o3a_ndx) = high_rel_err  ! MOD for CAM-SIMA: fix pre-existing CAM bug (was eps(m))
    end if
    m = get_spc_ndx( 'XNO' )
    if( m > 0 ) then
       eps(m) = high_rel_err
  ...
  subroutine imp_sol( base_sol, reaction_rates, het_rates, extfrc, delt, &
    ...
    if( cls_rxt_cnt(1,4) > 0 .or. extcnt > 0 ) then
       call indprd( 4, ind_prd, clscnt4, base_sol, extfrc, &
            reaction_rates, ncol, nlev )  ! MOD for CAM-SIMA: added nlev (pver) arg
    else
