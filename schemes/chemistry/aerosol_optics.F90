  ...
  subroutine aerosol_optics_run( &
    ...
    aerosol_model_loop: do iaermod = 1, num_aero_models
      ...
      bin_loop: do ibin = 1, nbins
        ...
        ! Only for volcanic_radius, get the geometric radius from the
        ! constituents array (host-model specific). The radius is per mode:
        ! the optics type carries the mode suffix (volcanic_radius1 ->
        ! VOLC_RAD_GEOM1), and the bulk optics type carries none.
        ! Source: CAM aerosol_optics_cam.
