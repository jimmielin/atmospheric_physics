! Surface precipitation rate diagnostics: the CAM PRECC/PRECL/PRECT family.
! CAM computes these in cam_diagnostics (diag_conv) by summing the per-process
! surface rates; this scheme reads the same per-process rates that
! set_surface_coupling_vars sends to the coupler, so the history fields are
! guaranteed to match what the surface components receive.
...
contains
   ...
   subroutine precipitation_diagnostics_init(errmsg, errflg)
      ...
      call history_add_field('PRECC',  'lwe_convective_precipitation_rate_at_surface', horiz_only, 'avg', 'm s-1') ! Convective precipitation rate (liq + ice)
      call history_add_field('PRECL',  'lwe_large_scale_precipitation_rate_at_surface', horiz_only, 'avg', 'm s-1') ! Large-scale (stable) precipitation rate (liq + ice)
      call history_add_field('PRECT',  'total_precipitation_rate_at_surface', horiz_only, 'avg', 'm s-1') ! Total (convective and large-scale) precipitation rate (liq + ice)
      call history_add_field('PRECSC', 'lwe_convective_snowfall_rate_at_surface', horiz_only, 'avg', 'm s-1') ! Convective snow rate (water equivalent)
      call history_add_field('PRECSL', 'lwe_snow_and_cloud_ice_precipitation_rate_at_surface_due_to_microphysics', horiz_only, 'avg', 'm s-1') ! Large-scale (stable) snow rate (water equivalent)
